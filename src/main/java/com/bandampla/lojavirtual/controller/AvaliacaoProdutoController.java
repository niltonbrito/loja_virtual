...
public class AvaliacaoProdutoController implements AvaliacaoProdutoControllerAPI {
	...
	public ResponseEntity<ResponseDefaultDTO<AvaliacaoProdutoDTO>> cadastrar(AvaliacaoProdutoDTO dto,
		...
		AvaliacaoProdutoDTO retorno = avaliacaoProdutoService.cadastrar(dto, usuarioLogado);

		ResponseDefaultDTO<AvaliacaoProdutoDTO> response = new ResponseDefaultDTO<>(HttpStatus.CREATED.toString(),
				"Avaliação do produto registrada com sucesso", retorno, request.getRequestURI(), traceId);
	...
	public ResponseEntity<ResponseDefaultDTO<AvaliacaoProdutoDTO>> buscarPorId(Long id,
		...
		String traceId = UUID.randomUUID().toString();
		AvaliacaoProdutoDTO retorno = avaliacaoProdutoService.buscarPorId(id, usuarioLogado);

		ResponseDefaultDTO<AvaliacaoProdutoDTO> response = new ResponseDefaultDTO<>(HttpStatus.OK.toString(),
				"Avaliação do produto recuperada com sucesso", retorno, request.getRequestURI(), traceId);
	...
	public ResponseEntity<List<AvaliacaoProdutoDTO>> buscarPorDescricao(String descricao,
			UsuarioLogadoPrincipal usuarioLogado) throws ExceptionCustom {
		return ResponseEntity.ok(avaliacaoProdutoService.buscarPorDescricao(descricao, usuarioLogado));
	}

	@Override
	public ResponseEntity<List<AvaliacaoProdutoDTO>> buscarTodosPorEmpresa(UsuarioLogadoPrincipal usuarioLogado)
			throws ExceptionCustom {
		return ResponseEntity.ok(avaliacaoProdutoService.buscarTodosPorEmpresa(usuarioLogado));
	}

	@Override
	public ResponseEntity<Page<AvaliacaoProdutoDTO>> buscarAvancado(String descricao, Integer nota, int page, int size,
			UsuarioLogadoPrincipal usuarioLogado) {
		return ResponseEntity.ok(avaliacaoProdutoService.buscarAvancado(descricao, nota, page, size, usuarioLogado));
	}

	@Override
	public ResponseEntity<Page<AvaliacaoProdutoDTO>> buscarPaginado(int page, int size, String sort, String direction,
			UsuarioLogadoPrincipal usuarioLogado) {
		return ResponseEntity.ok(avaliacaoProdutoService.buscarPaginado(page, size, sort, direction, usuarioLogado));
	}
}